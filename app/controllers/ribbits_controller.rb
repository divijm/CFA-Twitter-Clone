class RibbitsController < ApplicationController
  before_action :set_ribbit, only: [:show, :edit, :update, :destroy, :toggle_follow, :toggle_like]
  ...
  def toggle_follow
    @follow_status = current_frog.toggle_follow!(@ribbit.frog)
    redirect_to :back
  end

  def toggle_like
    @like_status = current_frog.toggle_like!(@ribbit)
    redirect_to :back
  end
  # POST /ribbits
  # POST /ribbits.json
  ...
  private
    ...
    def ribbit_params
      params.require(:ribbit).permit(:croak, :frog_id)
    end
end
